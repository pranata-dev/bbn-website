...
export default async function TryoutDiscussionPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (!submission) {
        redirect(`/dashboard/tryouts/${tryoutId}`)
    }
        ...
        <div className="max-w-4xl mx-auto space-y-6">
            <div className="flex items-center gap-4">
                <Button variant="ghost" size="icon" asChild className="hover:bg-warm-beige hover:text-dark-brown">
                    <Link href={`/dashboard/tryouts/${tryoutId}/result?score=${submission.score}&correct=${submission.correct_count}&total=${submission.total_count}`}>
                        <ArrowLeft className="w-5 h-5" />
                    </Link>
                </Button>
                <div>
                    <h1 className="text-2xl font-bold font-serif text-dark-brown">Pembahasan Tryout</h1>
