...
async function run() {
    console.log('--- SEEDING WEEK 5 (SUPABASE JS) ---');
    console.log(`Processing ${week5Questions.length} questions for WEEK_5...`);

    let inserted = 0;
    let skipped = 0;

    for (const q of week5Questions) {
    ...
    const { data: allW5, error: fetchErr } = await supabase
        .from('questions')
        .select('id')
        .eq('category', 'WEEK_5');

    if (fetchErr) {
        console.error(`Error fetching W5 questions: ${fetchErr.message}`);
    } else if (allW5) {
        for (let i = 0; i < allW5.length; i++) {
            const qid = allW5[i].id;
