...
async function run() {
    console.log('--- SEEDING WEEK 6 (SUPABASE JS) ---');
    console.log(`Processing ${week6Questions.length} questions for WEEK_6...`);

    let inserted = 0;
    let skipped = 0;

    for (const q of week6Questions) {
        // 1. Check if exists based on text
    ...
    const { data: allW6, error: fetchErr } = await supabase
        .from('questions')
        .select('id')
        .eq('category', 'WEEK_6');

    if (fetchErr) {
        console.error(`Error fetching W6 questions: ${fetchErr.message}`);
    } else if (allW6) {
        for (let i = 0; i < allW6.length; i++) {
            const qid = allW6[i].id;
