...
async function run() {
    console.log('--- SEEDING WEEK 7 (SUPABASE JS) ---');
    console.log(`Processing ${week7Questions.length} questions for WEEK_7...`);

    let inserted = 0;
    let skipped = 0;

    for (const q of week7Questions) {
    ...
    const { data: allW7, error: fetchErr } = await supabase
        .from('questions')
        .select('id')
        .eq('category', 'WEEK_7');

    if (fetchErr) {
        console.error(`Error fetching W7 questions: ${fetchErr.message}`);
    } else if (allW7) {
        for (let i = 0; i < allW7.length; i++) {
            const qid = allW7[i].id;
